...
| `REPLICA IDENTITY FULL` on `items` | With the default, a DELETE event carries only the primary key, RLS cannot evaluate the policy against the removed row, and the delete silently never reaches the other phone. |
